...
def conv2_1pix(pic_,kernel,i,j,M,N):
    value_out = 0
    [l,w] = kernel.shape
    off_l = int((l-1)/2)
    off_w = int((w-1)/2)
    for x in range(l):
        for y in range(w):
            if (i-off_l+x<0) or (j-off_w+y<0) or (i-off_l+x>=M) or (j-off_w+y >= N):
                pic_data=0
            else:
                #print (i-off_l+x,j-off_w+y)
                pic_data = pic_[i-off_l+x][j-off_w+y]
            pic_data = float(pic_data)    
            value_out += pic_data * kernel[x][y]
            #print (np.uint8(value_out))
    return int(value_out)

# Implement This funtion
def conv2(pic,kernel):
    # Create a new pic with same size but float type
    pic_conv = numpy.zeros(numpy.shape(pic), dtype=int)
    [M,N,T] = pic.shape
    print (M,N,T)
    print(kernel)
    ...
    for layer in range(3):
        ...
        for i in range (M):
            for j in range (N):
                pic_conv[i][j][layer] = int(conv2_1pix(pic_,kernel,i,j,M,N))
                
        print ("layer ",layer, "completed")
        print (pic_)
        print (pic_conv[:,:,layer])
    return pic_conv
